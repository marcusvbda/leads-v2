...
@section('content')
    ...
    if ($logged_user->hasRole(['super-admin', 'admin'])) {
        $roles = DB::table('roles')
            ->where('tenant_id', $logged_user->tenant_id)
    ...
    ?>
    <user-profile :user='@json($user)' :logged='@json($logged_user)'
        :departments='@json($departments)' :roles='@json($roles)'
        :polos='@json($polos)' :polos_ids='@json($polos_ids)'>
    </user-profile>
@endsection

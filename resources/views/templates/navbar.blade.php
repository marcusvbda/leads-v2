    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item {{ currentClass(['/admin']) }}">
                <a class="nav-link" href="/"><i class="el-icon-s-home mr-2"></i>Página Inicial<span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item {{ currentClass(['/admin/dashboard']) }}">
                <a class="nav-link" href="/admin/dashboard"><i class="el-icon-data-line mr-2"></i>Dashboard<span class="sr-only">(current)</span></a>
            </li>
            ...
            <li class="nav-item dropdown {{ currentClass(['/admin/relatorios/*']) }}">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    <a class="{{ getMenuClass('viewlist-leads', ['/admin/relatorios/leads/*']) }}" href="/admin/relatorios/leads" data-label="Relatório de Leads">Leads</a>
                </div>
            </li>
            @if($is_admin_or_super_admin)
            <li class="nav-item dropdown {{ currentClass(['/admin/webhooks/*','/admin/integracoes/*']) }}">
                <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="el-icon-s-claim mr-2"></i>Captação
                </a>
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    <a class="dropdown-item" href="/admin/webhooks" data-label="Rotas de Entrada de Lead">
                        Webhook
                    </a>
                    <a class="dropdown-item" href="/admin/integradores" data-label="Usuários de Acesso a API">
                        Integradores
                    </a>
                </div>
            </li>
            ...
            <li class="nav-item dropdown {{ currentClass(['/admin/resposta-contatos/*', '/admin/tipos-contato/*', '/admin/respostas-contato/*', '/admin/regra-classificacao/*', '/admin/objecoes/*']) }}">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    ...
                    <a class="{{ getMenuClass('config-rating-behavior', ['/admin/regra-classificacao/*']) }}" href="/admin/regra-classificacao" data-label="Regra de Rating de Lead">Regra de
                        Classificação
                    </a>
                    <a class="dropdown-item  {{ getMenuClass('viewlist-email-integrators',['/admin/integradores-de-email/*']) }}" href="/admin/integradores-de-email">
                        Integradores de Email
                    </a>
                </div>
            </li>
            <li class="nav-item dropdown {{ currentClass(['/admin/modelos-de-email/*']) }}">
                <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="el-icon-chat-dot-round mr-2"></i>Comunicação
                </a>
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                    <a class="dropdown-item {{ getMenuClass('viewlist-email-templates',['/admin/modelos-de-email/*']) }}" href="/admin/modelos-de-email/">
                        Modelos de Email
                    </a>
                </div>
            </li>
        </ul>
        <select-polo polo_name="{{ $polo->name }}" user_id="{{ $user->id }}" :logged_id='@json($polo->id)'>
        </select-polo>

    ...
    <div class="mb-4 info-obs">
        ...
        >
            ...
            >
                <el-step
                    title="Step 1"
                    description="Defina o tipo do contato"
                />
            ...
            <div class="d-flex flex-column my-4 mt-5">
                <template v-if="form_new_contact.step == 0">
                    ...
                    <small class="text-muted"
                        >Selecione o meio que o operador utilizará para efetuar
                        o contato com o lead</small
                    >
                    <hr />
                    <span
                        slot="footer"
                ...
                <template v-if="form_new_contact.step == 1">
                    ...
                    <el-select
                        v-model="form_new_contact.answer_id"
                        placeholder="Informe a resposta do contato"
                    >
                        <el-option-group
                            v-for="(group, g) in grouped_answer_types"
                            :key="g"
                            :label="group.type"
                        >
                            <el-option
                    ...
                    <small class="text-muted"
                        >Selecione a resposta do contato referente a proposta
                        realizada</small
                    >
                    <hr />
                    <span
                ...
                <template v-if="form_new_contact.step == 2">
                    ...
                    <el-select
                        v-model="form_new_contact.objection_id"
                        placeholder="Selecione a objeção do contato"
                    >
                        <el-option
                            v-for="(ob, i) in objection_options"
                            :key="i"
                    ...
                    <div class="mt-3 d-flex flex-column">
                        ...
                        />
                        <small class="text-muted"
                            >Dependendo da objeção escolhida, o detalhamento é
                            obrigatório</small
                        >
                    </div>
                    <hr />
                ...
                <template v-if="form_new_contact.step == 3">
                    <label
                        v-if="selected_answer.behavior == 'need_schedule_test'"
                    >
                        Selecione informações para agendamento do vestibular
                    </label>
                    <label v-else
                        >Selecione informações para reagendamento de
                        contato</label
                    >
                    <el-date-picker
                    ...
                    <small
                        ...
                        v-if="selected_answer.behavior == 'need_schedule_test'"
                    >
                        Nesta data e hora o lead deverá comparecer para realizar
                        a prova
                    </small>
                    <small class="text-muted" v-else>
                        Nesta data e hora seu usuário será notificado para
                        efetuar um novo contato com este lead
                    </small>
                    <hr />
                    <span
                        slot="footer"
                    ...
                    <div class="jumbotron">
                        <div class="row">
                            <div class="col-md-6 col-sm-12">
                                ...
                                <p class="mb-0">
                                    <b>Tipo de Contato : </b
                                    >{{ selected_contact_type.description }}
                                </p>
                                <p
                                    class="mb-0"
                                    v-if="form_new_contact.schedule"
                                >
                                    <b>Agendamento : </b
                                    >{{ formatDate(form_new_contact.schedule) }}
                                </p>
                                <p class="mb-0">
                                    <b>Tipo de Resposta : </b
                                    >{{ selected_answer.f_type }}
                                </p>
                                <p class="mb-0">
                                    <b>Resposta do Contato : </b
                                    >{{ selected_answer.description }}
                                </p>
                                <p class="mb-0" v-if="selected_objection">
                                    <b>Objeção : </b
                                    >{{ selected_objection.description }}
                                </p>
                                <p
                                    class="mb-0"
                                    v-if="form_new_contact.other_objection"
                                >
                                    <b>Descrição da Objeção : </b
                                    >{{ form_new_contact.other_objection }}
                                </p>
                            </div>
                            <div class="col-md-6 col-sm-12">
                                <template v-if="hasActionSendEmail">
                                    <InputTemplateEmail
                                        v-model="form_new_contact.sending_email"
                                    />
                                    <hr />
                                </template>
                                <label
                                    >Observações<small class="text-muted ml-2"
                                ...
                                />
                                <small class="text-muted">
                                    Digite aqui caso queira adicionar alguma
                                    observação sobre este contato
                                </small>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12">
                <el-tabs type="border-card">
                        ...
                        <el-timeline class="pt-3">
                            <el-timeline-item>
                                <p class="f-12 text-muted">
                                    ...
                                    >
                                        <span
                                            class="el-icon-circle-plus mr-1"
                                        />Novo Contato
                                    </a>
                                </p>
                            </el-timeline-item>
                            <template v-if="lead.tries.length > 0">
                                <el-timeline-item
                                    v-for="(t, i) in lead.tries"
                                    :key="i"
                                    :timestamp="`${t.date} - ${t.timestamp}`"
                                >
                                    <div class="f-12 text-muted">
                                        <p class="mr-1 mb-0 f-12" v-if="t.type">
                                            Tipo : <b v-html="t.type" />
                                        </p>
                                        <p class="mr-1 mb-0 f-12" v-if="t.user">
                                            Por : <b v-html="t.user" />
                                        </p>
                                        <p class="mr-1 mb-0 f-12" v-if="t.obs">
                                            Observações : <b v-html="t.obs" />
                                        </p>
                                        <p
                                            class="mr-1 mb-0 f-12"
                                            v-if="t.comment"
                                        >
                                            Comentário :
                                            <b v-html="t.comment" />
                            ...
                            <div class="text-muted text-center f-12" v-else>
                                Nenhuma contato anterior
                            </div>
                        </el-timeline>
                    </el-tab-pane>
                    <el-tab-pane label="Conversões Anteriores">
                        <el-timeline
                            class="pt-3"
                            v-if="lead.conversions.length > 0"
                        >
                            <el-timeline-item
                            ...
                            >
                                ...
                                <p
                                    ...
                                    v-if="conversion.user"
                                >
                                    <span class="mr-1"
                                        >Por : <b v-html="conversion.user"
                                    /></span>
                                </p>
                            </el-timeline-item>
                        </el-timeline>
                        <div class="text-muted text-center f-12" v-else>
                            Nenhuma conversão anterior
                        </div>
                    </el-tab-pane>
                    <el-tab-pane
                        label="Lead Completo (Webhook)"
                        v-if="lead.lead_api"
                    >
...
export default {
    ...
    computed: {
        ...
        selected_objection() {
            ...
            return this.objection_options.find(
                (x) => x.id == this.form_new_contact.objection_id
            );
        },
        selected_answer() {
            if (!this.form_new_contact.answer_id) return;
            return this.answers.find(
                (x) => x.id == this.form_new_contact.answer_id
            );
        },
        selected_contact_type() {
            if (!this.form_new_contact.type_id) return;
            return this.contact_types.find(
                (x) => x.id == this.form_new_contact.type_id
            );
        },
        ...
        hasActionSendEmail() {
            return (
                this.selected_contact_type.action == 'send-email' &&
                this.lead.email
            );
        },
    },
    methods: {
        ...
        validEmailSend() {
            if (this.hasActionSendEmail) {
                if (this.form_new_contact.sending_email.type == 'custom') {
                    if (
                        this.form_new_contact.sending_email.body &&
                        !this.form_new_contact.sending_email.subject
                    ) {
                        throw this.$message.error(
                            'Para enviar email digite o assunto !!'
                        );
                    }
                    ...
                    if (
                        ...
                        throw this.$message.error(
                            'Para enviar email digite o corpo !!'
                        );
                    }
                    ...
                    if (
                        !this.form_new_contact.sending_email.body ||
                        (!this.form_new_contact.sending_email.subject &&
                            !this.form_new_contact.sending_email.integrator_id)
                    ) {
                        throw this.$message.error(
                            'Para enviar email selecione o integrador !!'
                        );
                    }
                } else {
                    if (
                        this.form_new_contact.sending_email.integrator_id &&
                        !this.form_new_contact.sending_email.template_id
                    ) {
                        throw this.$message.error(
                            'Para enviar email selecione o template !!'
                        );
                    }
        ...
        cancelStepFour() {
            ...
            if (answer.behavior == 'need_objection')
                return this.clearScheduleAndObjectionsAndGoTo(2);
            if (
                ['need_schedule', 'need_schedule_test'].includes(
                    answer.behavior
                )
            )
        ...
        finishStepFour() {
            if (!this.form_new_contact.schedule)
        ...
        finishStepThree() {
            if (!this.form_new_contact.objection_id)
                return this.$message.error('Selecione a objeção do contato');
            let objection = this.selected_objection;
            if (objection.need_description) {
                if (!this.form_new_contact.other_objection)
                    return this.$message.error(
                        'Informe a descrição da objeção do contato'
                    );
            }
            this.form_new_contact.step = 4;
        },
        ...
        finishStepOne() {
            if (!this.form_new_contact.type_id)
                return this.$message.error('Selecione o tipo do contato');
